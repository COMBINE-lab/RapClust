...
void CollapsedEMOptimizer::clusterTranscripts(ReadExperiment& readExp){
    ...
    std::vector<double> weightEq ;
    weightEq.resize(numEqClasses) ;
   ...
   if (BOOST_LIKELY(groupSize > 1)) {
                for (size_t i = 0; i < groupSize-1; ++i) {
                    ...
                    for (size_t j = i+1; j < groupSize; ++j){
                        ...
                        if( it != adjList.end()){
                            ...
                            it->second += eij_ / std::min(transcripts[tid_u].estCount(),transcripts[tid_v].estCount()) ;
                        }else{
                            adjList[key] =  eij_ / std::min(transcripts[tid_u].estCount(),transcripts[tid_v].estCount()) ;
                        }
                    }
                }
            }else{
                auto key = std::make_tuple(txps[0],txps[0]) ;
                adjList[key]= (2 * count )/ transcripts[txps[0]].estCount() ;
            }
        }
    }
    int cc = 0 ;
    for (auto& p:adjList){
       ...
       if (std::get<0>(key) == std::get<1>(key)){
            ...
            fp << transcripts[std::get<0>(key)].RefName << "\t" <<ref_name_dummy << "\t" << p.second << "\n" ;
