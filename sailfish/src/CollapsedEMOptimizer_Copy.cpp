...
void CollapsedEMOptimizer::clusterTranscripts(ReadExperiment& readExp){
    ...
    std::unordered_map<keyT,uint64_t, KeyPairHasher> adjList;

    for(size_t eqID = 0; eqID < numEqClasses ; ++eqID) {
        auto& kv = eqVec[eqID] ;
        uint64_t count = kv.second.count;
        const TranscriptGroup& tgroup = kv.first;
        //for each equivalence class find sum(exp)
    }
    ...
    for (size_t eqID = 0; eqID < numEqClasses; ++eqID) {
        ...
        if(tgroup.valid){
            ...
            if (BOOST_LIKELY(groupSize > 1)) {
                for (size_t i = 0; i < groupSize; ++i) {
                    weightTr.insert(transcripts[txps[i]].estCount()/transcripts[txps[i]].EffectiveLength) ;
